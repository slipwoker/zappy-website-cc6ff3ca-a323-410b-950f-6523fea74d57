document.addEventListener('DOMContentLoaded', function () {
  ...
  if (contactForm) {
    ...
    const getErrorMessage = function (field, ruleName, ruleValue) {
      const label =
    ...
    const validateField = function (field) {
      ...
      for (let i = 0; i < rules.length; i++) {
        ...
        if (validators[ruleName] && !validators[ruleName](field.value, ruleValue)) {
    ...
    contactForm.addEventListener('submit', function (e) {
      ...
      fetch(action, {
        ...
        .then(function (response) {
          if (response.ok) {
            contactForm.reset();
            contactForm.querySelectorAll('.input-success').forEach(function (el) {
  ...
  if ('IntersectionObserver' in window) {
    const scrollObserver = new IntersectionObserver(onIntersect, animationObserverOptions);
    ...
    document.querySelectorAll(OBSERVED_SELECTOR).forEach(function (el) {
      el.classList.add(ANIMATION_CLASS);
    });
  }
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "לראות את הים (See the Sea) משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: 'cc6ff3ca-a323-410b-950f-6523fea74d57',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || 'cc6ff3ca-a323-410b-950f-6523fea74d57';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = 'cc6ff3ca-a323-410b-950f-6523fea74d57';
